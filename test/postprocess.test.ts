...
describe('presetUseful postprocess with important', () => {
  ...
  it('base with includes', async () => {
    ...
    expect(css).toMatchInlineSnapshot(`
      ...
      /* layer: default */
      .\\!text-xl{font-size:var(--text-xl-fontSize);line-height:var(--un-leading, var(--text-xl-lineHeight));}
      .text-blue{color:color-mix(in srgb, var(--colors-blue-DEFAULT) var(--un-text-opacity), transparent) !important;}
      .important-ma{margin:auto !important;}
      .bg-red{background-color:color-mix(in srgb, var(--colors-red-DEFAULT) var(--un-bg-opacity), transparent) !important;}
      @supports (color: color-mix(in lab, red, red)){
      .text-blue{color:color-mix(in oklab, var(--colors-blue-DEFAULT) var(--un-text-opacity), transparent) !important;}
      .bg-red{background-color:color-mix(in oklab, var(--colors-red-DEFAULT) var(--un-bg-opacity), transparent) !important;}
      }
      @media (min-width: 40rem){
      .sm\\:text-sm\\!{font-size:var(--text-sm-fontSize);line-height:var(--un-leading, var(--text-sm-lineHeight));}
      }"
